...
$TmpPkg = Join-Path $OpenClawHome ".bbot-plugin.tmp.$Ts.tgz"
...
function Resolve-Manifest {
  Step "读取版本清单 -> $ManifestUrl"
  $manifest = Invoke-RestMethod -Uri $ManifestUrl
  if ($Version -eq "latest") {
    $resolved = [string]$manifest.latest
  } else {
    $resolved = [string]$Version
  }
  if ([string]::IsNullOrWhiteSpace($resolved)) {
    throw "manifest 缺少 latest 字段。"
  }

  $entry = $manifest.versions.PSObject.Properties[$resolved].Value
  if ($null -eq $entry) {
    throw "manifest 未包含版本 $resolved。"
  }
  if ([string]::IsNullOrWhiteSpace([string]$entry.tarballUrl) -or [string]::IsNullOrWhiteSpace([string]$entry.sha256)) {
    throw "manifest 版本 $resolved 缺少 tarballUrl 或 sha256。"
  }

  $script:ResolvedVersion = $resolved
  $script:TarballUrl = [string]$entry.tarballUrl
  $script:ExpectedSha256 = ([string]$entry.sha256).ToLowerInvariant()
}

function Download-AndVerifyPackage {
  Resolve-Manifest
  Step "下载插件包 -> $TarballUrl"
  Invoke-WebRequest -Uri $TarballUrl -OutFile $TmpPkg
  $actual = (Get-FileHash -LiteralPath $TmpPkg -Algorithm SHA256).Hash.ToLowerInvariant()
  if ($actual -ne $ExpectedSha256) {
    throw "插件包哈希校验失败：expected=$ExpectedSha256 actual=$actual"
  }
  Step "插件包哈希校验通过（$ResolvedVersion）"
}
  ...
  if ($null -ne $cfgObj.plugins) {
    if ($cfgObj.plugins.allow -is [System.Array]) {
      $cfgObj.plugins.allow = @(
        $cfgObj.plugins.allow | Where-Object {
          $_ -ne "bbot" -and $_ -ne "openclaw-onebot" -and $_ -ne "@hmjbill/bbot" -and $_ -ne "@hmjbill/openclaw-onebot"
        }
      )
    }
    if ($null -ne $cfgObj.plugins.entries) {
      $cfgObj.plugins.entries.PSObject.Properties.Remove("bbot")
      $cfgObj.plugins.entries.PSObject.Properties.Remove("openclaw-onebot")
      $cfgObj.plugins.entries.PSObject.Properties.Remove("@hmjbill/bbot")
      $cfgObj.plugins.entries.PSObject.Properties.Remove("@hmjbill/openclaw-onebot")
    }
  }
...
function Sync-InstallMetadata {
  ...
  if ($null -eq $existing) {
    $existing = $cfgObj.plugins.installs.'openclaw-onebot'
  }
  if ($null -eq $existing) {
  ...
  $existing | Add-Member -NotePropertyName "spec" -NotePropertyValue "@hmjbill/bbot" -Force
  ...
  $existing | Add-Member -NotePropertyName "resolvedName" -NotePropertyValue "@hmjbill/bbot" -Force
  $existing | Add-Member -NotePropertyName "resolvedVersion" -NotePropertyValue $pkgObj.version -Force
  $existing | Add-Member -NotePropertyName "resolvedSpec" -NotePropertyValue "@hmjbill/bbot@$($pkgObj.version)" -Force
...
try {
  ...
  Download-AndVerifyPackage
  Step "安装版本 $Pkg@$ResolvedVersion"
  ...
  if (Test-Path -LiteralPath $TmpPkg) {
    Remove-Item -LiteralPath $TmpPkg -Force -ErrorAction SilentlyContinue
  }
}
